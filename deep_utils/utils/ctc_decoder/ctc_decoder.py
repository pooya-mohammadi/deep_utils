...
class CTCDecoder:
    ...
    def greedy_decode(log_prob: np.ndarray, blank_class=0, **kwargs):
        ...
        labels = CTCDecoder.merge_duplicates_remove_blanks(labels, blank_class=blank_class)
        return labels
    ...
    def beam_search_decode(emission_log_prob, blank_class=0, **kwargs):
        ...
        emission_threshold = kwargs.get('emission_threshold', np.log(CTCDecoder.DEFAULT_EMISSION_THRESHOLD))
        ...
        for prefix, accu_log_prob in beams:
            labels = tuple(CTCDecoder.merge_duplicates_remove_blanks(prefix, blank_class=blank_class))
            # log(p1 + p2) = logsumexp([log_p1, log_p2])
            total_accu_log_prob[labels] = \
                logsumexp([accu_log_prob, total_accu_log_prob.get(labels, CTCDecoder.NINF)])
    ...
    def prefix_beam_decode(emission_log_prob, blank=0, **kwargs):
        ...
        beams = [(tuple(), (0, CTCDecoder.NINF))]  # (prefix, (blank_log_prob, non_blank_log_prob))
        # initial of beams: (empty_str, (log(1.0), log(0.0)))

        for t in range(length):
            new_beams_dict = defaultdict(lambda: (CTCDecoder.NINF, CTCDecoder.NINF))  # log(0.0) = NINF
    ...
    @staticmethod
    def ctc_decode_batch(log_probs: np.ndarray, label2char=None, blank_class=0, decoder_name='greedy', beam_size=10):
        """
        ...
        decoded_list = [CTCDecoder.ctc_decode(log_prob, decoder_name, blank_class, label2char, beam_size) for log_prob
                        in log_probs]
        return decoded_list

    @staticmethod
    def ctc_decode(log_prob: np.ndarray, decoder_name="greedy", blank_class=0, label2char=None, beam_size=10):
        decoder = {
            'greedy': CTCDecoder.greedy_decode,
            'beam_search': CTCDecoder.beam_search_decode,
            'prefix_beam_search': CTCDecoder.prefix_beam_decode,
        }[decoder_name]
        decoded = decoder(log_prob, blank_class=blank_class, beam_size=beam_size)
        if label2char:
