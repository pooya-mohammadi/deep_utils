...
def repeat_dimension(input_array: np.ndarray, n=3, d=2, logger=None, verbose=0):
    ...
    if not isinstance(input_array, np.ndarray):
        ...
        value_error_log(logger, f"input_array.shape:{input_array.shape} is lower the one:{d} which should be repeated!")
    result = np.repeat(input_array, n, axis=d)
    log_print(logger, f"Successfully create result.shape: {result.shape}!", verbose=verbose)
    return result
