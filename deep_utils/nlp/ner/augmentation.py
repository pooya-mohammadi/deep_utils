...
class NERAugRemove(NERAugmentation):
    ...
    def __init__(self,
                 ...
                 ner_type="BIO", make_copy=True, random_seed=None):
        """
            ...
            for query_tokens, query_label in self.augmentation_len_dict[token_len]:
                ...
                while token_index <= (len(tokens_x) - token_len):
                    ...
                    if query_tokens == tuple(selected_tokens) and (
                        ...
                        remove_label = labels_y[token_index: token_index + token_len][0]
                        if remove_label.startswith("B"):
                            ...
                            if next_index < len(labels_y) and labels_y[next_index].startswith("I") and labels_y[
                                                                                                           next_index][
                                                                                                       2:] == query_label:
                                labels_y[next_index] = "B-" + query_label  # start with B
...
class NERAugReplacement(NERAugmentation):
    ...
    def __call__(self, tokens_x: Union[Tuple[str], List[str]], labels_y: Union[Tuple[str], List[str]]):
        if self.make_copy:
            tokens_x, labels_y = deepcopy(tokens_x), deepcopy(labels_y)
        super().__call__(tokens_x, labels_y)
        for token_len in self.aug_tokens_lengths:
