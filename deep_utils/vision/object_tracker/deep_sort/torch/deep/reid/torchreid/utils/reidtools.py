...
def visualize_ranked_results(
    ...
    def _cp_img_to(src, dst, rank, prefix, matched=False):
        ...
        if isinstance(src, (tuple, list)):
            if prefix == "gallery":
                ...
                dst = osp.join(dst, prefix + "_top" +
                               str(rank).zfill(3)) + "_" + suffix
            else:
            ...
            dst = osp.join(
                dst, prefix + "_top" +
                str(rank).zfill(3) + "_name_" + osp.basename(src)
            )
            shutil.copy(src, dst)
    ...
    for q_idx in range(num_q):
        ...
        if data_type == "image":
            ...
            qdir = osp.join(save_dir, osp.basename(
                osp.splitext(qimg_path_name)[0]))
