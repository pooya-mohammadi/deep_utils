...
class MTCNNTorchFaceDetector(FaceDetector):
    ...
    def detect_faces(
        ...
        img_boxes = torch.FloatTensor(np.concatenate(image_boxes)).to(
            self.config.device
        )
        ...
        for img_n in range(img.shape[0]):
            offsets = (
                output[0].cpu().data.numpy()[split[img_n]: split[img_n + 1]]
            )  # shape [n_boxes, 4]
            probs = (
                output[1].cpu().data.numpy()[split[img_n]: split[img_n + 1]]
            )  # shape [n_boxes, 2]
        ...
        for img_n in range(img.shape[0]):
            ...
            landmarks = output[0].cpu().data.numpy()[
                split[img_n]: split[img_n + 1]]
            offsets = (
                output[1].cpu().data.numpy()[split[img_n]: split[img_n + 1]]
            )  # shape [n_boxes, 4]
            probs = (
                output[2].cpu().data.numpy()[split[img_n]: split[img_n + 1]]
            )  # shape [n_boxes, 2]
        ...
        output = self.output_class(
            boxes=boxes_, confidences=confidences_, landmarks=landmarks_
        )
        return output
