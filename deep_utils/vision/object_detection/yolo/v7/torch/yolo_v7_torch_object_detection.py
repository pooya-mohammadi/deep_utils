...
class YOLOV7TorchObjectDetector(YOLOObjectDetector):
    ...
    def load_model(self):
        ...
        ROOT = str(FILE.parents[0])  # YOLOv7 root directory
        if ROOT not in sys.path:
            sys.path.append(ROOT)  # add ROOT to PATH
            v5_root = ROOT.replace("v7", "v5")
            if v5_root in sys.path:
                sys.path.remove(v5_root)
    ...
    def detect_objects(
        ...
        :return:
        """
        from .yolov7_utils.general import non_max_suppression, scale_coords
